...
class HB(Broker_Connection):
    ...
    def connect(self, hb=None):
        for i in self.attempts:
            if self.connected(hb): break # si ya está conectado salgo
    ...
    def get_market_snapshot(self, board:str, settlement:int=0):
        ...
        self.connect(self.hb_connection)
        if board == 'options':
            return self.hb_connection.online._scrapping.get_options()    
        if board in set(['accionesLideres', 'panelGeneral', 'cedears', 'rentaFija', 'letes', 'obligaciones']) and settlement in set([0,1,2]):
            return self.hb_connection.online._scrapping.get_securities(board, settlement+1)
        print(' Los pararmetros son incorrectos')
