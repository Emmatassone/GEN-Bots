...
##### CONFIG FILES #####
#########################
def config_read(file_name:str='config.ini', abspath:str='', relative_path:str=''):
    if relative_path != '':
        if not relative_path.startswith('\\'): relative_path= '\\'+relative_path
        if not relative_path.endswith('\\'):   relative_path= relative_path+'\\'
        path = os.getcwd()+relative_path
    else:
        path =  abspath+('\\' if abspath != '' and not abspath.endswith('\\') else '')
    config = configparser.ConfigParser()
    reading = config.read(path+file_name)
    if reading != [path+file_name]:
        raise Exception(f' ConfigReadingError: El archivo {file_name} no ha podido ser leido correctamente.')
    return config
