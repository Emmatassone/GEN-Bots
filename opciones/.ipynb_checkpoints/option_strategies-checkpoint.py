    ...
    def bull_call_spread(self, order_by='Ganancia Maxima', rows=10):
       ...
       Returns:
        ...
        for strike1, strike2 in list(itertools.product(strikes, repeat=2)):
            ...
            self.bull_call_spreads.append(spread)

        return pd.DataFrame(self.bull_call_spreads).sort_values(by=order_by, ascending=False).dropna().head(rows)
