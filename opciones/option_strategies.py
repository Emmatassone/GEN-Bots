        ...
        for strike in self.options_df['strike'].unique():
            ...
            for call1, call2 in itertools.combinations(call_options.index, 2):
                spread = {
                    ...
                    'MaxProfit': self.options_df.loc[call2, 'strike'] - strike - (
                                self.options_df.loc[call1, 'ask'] - self.options_df.loc[call2, 'ask'])
                }
        ...
        for strike in self.options_df['strike'].unique():
            ...
            for put1, put2 in itertools.combinations(put_options.index, 2):
                spread = {
                    ...
                    'MaxProfit': strike - self.options_df.loc[put1, 'strike'] - (
                                self.options_df.loc[put1, 'ask'] - self.options_df.loc[put2, 'ask'])
                }
