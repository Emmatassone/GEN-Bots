...
class OptionStrategyBuilder:
    ...
    def bull_call_spread(self, order_by='Ganancia Maxima', rows=10):
       ...
       Returns:
        ...
        return pd.DataFrame(bull_call_spreads).sort_values(by=order_by, ascending=False).head(rows).reset_index(
            drop=True)
    ...
    def bear_call_spread(self, order_by='Ganancia Maxima', rows=10):
        ...
        return pd.DataFrame(bear_call_spreads).sort_values(by=order_by, ascending=False).head(rows).reset_index(
            drop=True)
    ...
    def bull_put_spread(self, order_by='Ganancia Maxima', rows=10):
        ...
        return pd.DataFrame(bull_put_spreads).sort_values(by=order_by, ascending=False).head(rows).reset_index(
            drop=True)
    ...
    def bear_put_spread(self, order_by='Ganancia Maxima', rows=10):
        ...
        return pd.DataFrame(bear_put_spreads).sort_values(by=order_by, ascending=False).head(rows).reset_index(
            drop=True)
